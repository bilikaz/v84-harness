...
| Term | Meaning |
|------|---------|
| **service** | What the caller names: `main` (the chat provider) or a media slot (`imageGen`, `imageEdit`, `videoGen`, `imageRec`, `videoRec`, `audioGen`, `audioRec`). `SERVICE_MODALITY` maps each to its modality — recognition is a *text* interaction over a vision model; generation is its medium. `imageEdit` is generation WITH reference images (`GenParams.images`) — the OpenAI-images dialect routes it to `/images/edits` as multipart, reference files sent as REPEATED `image` fields (FastAPI-style servers bind `List[UploadFile]`; the `image[]` spelling 422s — [ADR-0076](../adr/0076-image-edit-service-and-referenceable-images.md)). |
