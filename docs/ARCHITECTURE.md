...
| `src/core/graph/` | The graph orchestration engine ([ADR-0067](adr/0067-graph-orchestration-engine.md)): `GraphEngine` (`ctx.graph`) drives event-driven named-node graphs (`Start`/`End` pairs, `goTo`/`splitTo`/`goToAll`/`done`, producer-declared arrival-driven joins, seeded heads + JSON heal, stop/continue on children), the `BaseGraph` base, the registry, and the `Select` user-resolution bridge |
...
| [architecture/graph.md](architecture/graph.md) | Graph orchestration: event-driven Start/End nodes; routes (goTo/splitTo/goToAll/done); arrival-driven joins; seeded heads + JSON heal; the Select primitive; stop/continue; plugin graphs + agents |
