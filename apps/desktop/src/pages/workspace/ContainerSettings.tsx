...
export function ContainerSettings(props: { container: Container; onClose: () => void; onDelete: () => void }) {
  ...
  function save() {
    const patch: Partial<Container> = { name: name.trim() || container.name, permissions: perms };
    if (container.type === "local") patch.config = { ...container.config, root: root.trim() };
    void updateContainer(container.id, patch);
    onClose();
  }
