import type { CallContext, ResponseHandler, ConfigLLM, GenParams, ModelInfo, ModelService } from "../types.ts";
import { SERVICE_MODALITY, targetLabel } from "../types.ts";
import type { BaseProvider } from "../providers/base.ts";
import { errorMessage } from "../../lib/errors.ts";
import { HealError, type CallOptions, type LLMClient, type ConfigSource } from "./types.ts";

export { HealError } from "./types.ts";
export type { CallOptions, LLMClient, ConfigSource } from "./types.ts";
...
type ProviderCtor = (new (target: ConfigLLM, ctx: CallContext) => BaseProvider) & {
...
export async function listProviderModels(provider: ConfigLLM["provider"]): Promise<ModelInfo[]> {
  const ctor = PROVIDER_MODULES[`../providers/text/${provider.type}.ts`]?.Provider;
  return ctor?.listModels ? ctor.listModels(provider) : [];
}

export function createClient(cxt: ConfigSource, defaults?: { maxHeals?: number }): LLMClient {
  function tuned(target: ConfigLLM, params?: GenParams): ConfigLLM {
  ...
  function resolveProvider(service: ModelService, ctx: CallContext): BaseProvider {
    const target = cxt.resolve(service);
    if (!target) {
